...
const createUser = (name: string, age: number, profession: string): object =>
  ({ name, age, profession })
...
let buttonUpdate = document.querySelector('#atualizar-saldo') as HTMLScriptElement;
let buttonClear = document.querySelector('#limpar-saldo') as HTMLScriptElement;
let sum = <HTMLInputElement>document.querySelector('#soma');
let balanceField = document.querySelector('#campo-saldo');

balanceField!.textContent = '0'

const addToTheBalance = (sum: string) => {
  balanceField!.textContent = `${sum}`
}

const clearBalence = (): void => {
  balanceField!.textContent = '0';
}
